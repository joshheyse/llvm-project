...
if(GRPC_INSTALL_PATH)
  ...
  # the CMake files telling us *which* static libraries to link are not.
  if(NOT BUILD_SHARED_LIBS)
    message(
      NOTICE
      "gRPC and Protobuf will be linked dynamically. If you want static linking, build gRPC from sources with -DBUILD_SHARED_LIBS=Off."
    )
  endif()
  find_program(GRPC_CPP_PLUGIN grpc_cpp_plugin)
  find_program(PROTOC protoc)
  if(NOT GRPC_CPP_PLUGIN OR NOT PROTOC)
    message(
      FATAL_ERROR
        "gRPC C++ Plugin and Protoc must be on $PATH for gRPC-enabled build.")
  endif()
  ...
  if(NOT TARGET grpc++)
    ...
    if(ENABLE_GRPC_REFLECTION)
      ...
      set_target_properties(
        grpc++_reflection PROPERTIES IMPORTED_LOCATION
...
# Proto headers are generated in ${CMAKE_CURRENT_BINARY_DIR}. Libraries that use
# these headers should adjust the include path. If the "GRPC" argument is given,
# services are also generated. The DEPENDS list should name *.proto source files
# that are imported. They may be relative to the source dir or absolute (for
# generated protos).
function(generate_proto_sources GeneratedSource ProtoFile)
  ...
  set(Flags --cpp_out="${CMAKE_CURRENT_BINARY_DIR}"
            --proto_path="${ProtoSourcePath}")
  if(PROTO_GRPC)
    list(APPEND Flags --grpc_out="${CMAKE_CURRENT_BINARY_DIR}"
         --plugin=protoc-gen-grpc="${GRPC_CPP_PLUGIN}")
    list(APPEND GeneratedProtoSource
         "${CMAKE_CURRENT_BINARY_DIR}/${Basename}.grpc.pb.cc")
    list(APPEND GeneratedProtoHeader
         "${CMAKE_CURRENT_BINARY_DIR}/${Basename}.grpc.pb.h")
  endif()
  add_custom_command(
    OUTPUT ${GeneratedProtoSource} ${GeneratedProtoHeader}
    COMMAND ${PROTOC} ARGS ${Flags} "${ProtoSourceAbsolutePath}"
  ...
  foreach(ImportedProto IN LISTS PROTO_DEPENDS)
    ...
    get_filename_component(ImportedHeader "${ImportedHeader}" ABSOLUTE BASE_DIR
    ...
    foreach(Generated IN LISTS GeneratedProtoSource)
      ...
      set_source_files_properties("${Generated}" PROPERTIES OBJECT_DEPENDS
                                                            "${ImportedHeader}")
